...
- **Degraded resolution improvisation:** "it's working but a bit slow" was not covered by the resolution prompt, so the model offered further WiFi tips instead of closing. Added an explicit "working but degraded" case: acknowledge the improvement, mention ISP if slowness continues, say goodbye.
