...
async function processGuidedSteps(
  ...
  if (isLastStep) {
    // Don't use stepsComplete — it can say goodbye, but next state is resolution not closed.
    // Finishing the last step is not the same as confirming the issue is fixed; ask explicitly.
